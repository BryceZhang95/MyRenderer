...
public:
    ...
    void scanConversion(Matrix t,int colorID)
    {
        ...
        for (int i=0; i<front_faces_number; i++)
        {
            ...
            for (int y=0; y<PIXELY; y++)
            {
                ...
                double gray = Phong_illumination(0.1, 0.8, 0.8, 1, 20,100, 1, 400, front_faces[i].normal, light_position, camera_position);
                //double gray_next = specular_term(0.8, 100, points[p_next].normal, light_position, camera_position, 50);
                if(gray< 0) gray = 0;
                if(gray > 25) gray = 25;
                double inten = (double)gray/25 ;
                //printf(">%f\n",inten);
                //printf(">%f %f %f\n",front_faces[i].normal.x_position,front_faces[i].normal.y_position,front_faces[i].normal.z_position);
                //printf(">%f\n",gray);
    ...
    void drawGouraud(Matrix t,int colorID)
    {
        ...
        for (int i=0; i<front_faces_number; i++)
        {
            ...
            for (int k=0; k<length; k++)
            {
                ...
                double gray_current = Phong_illumination(0.1, 0.8, 0.8, 1, 20,100, 1, 400, points[p_current].normal, light_position, camera_position);
                // double gray_current = specular_term(0.8, 100, points[p_current].normal, light_position, camera_position, 50);
                //printf(">%f\n",gray_current);
                ...
                double gray_next = Phong_illumination(0.1, 0.8, 0.8, 1, 20,100, 1, 400, points[p_next].normal, light_position, camera_position);
                //double gray_next = specular_term(0.8, 100, points[p_next].normal, light_position, camera_position, 50);
                ...
                for (int k=0; k<activeEdgeTable.size(); k+=2) {
                    ...
                    for (int x=(int)ceil(activeEdgeTable[k].minX); x<activeEdgeTable[k+1].minX; x++) {
                        ...
                        switch (colorID) {
                            case 1:
                            {
                                auto r = 0.8*intensity;
                                auto g = 0.8*intensity;
                                auto b = 0.8*intensity;
                                setFrameBuffer(y,x,z,r,g,b);
                                z+=dZ;
                                intensity+=dI;
                            }
                    ...
                    for (int x=(int)ceil(activeEdgeTable[k].minX); x<activeEdgeTable[k+1].minX; x++) {
                        ...
                        switch (colorID) {
                            case 1:
                            {
                                ...
                                //
                                //                                double inten = (double)gray / 100.0;
                                double gray = Phong_illumination(0.1, 0.8, 0.8, 1, 20,100, 1, 400, n, light_position, camera_position);
                                //double gray_next = specular_term(0.8, 100, points[p_next].normal, light_position, camera_position, 50);
                                if(gray< 0) gray = 0;
                                if(gray > 25) gray = 25;
                                double inten = (double)gray/25 ;
    ...
    void computeNormal(Matrix t)
    {
        for (int i=0; i<vertices_number; i++) {
            points[i].normal=Vector3();
        }
        for (int i=0; i<front_faces_number; i++) {
            ...
            front_faces[i].normal = (points[p2].vert*(t) - points[p1].vert*(t)).cross(points[p1].vert*(t) - points[p0].vert*(t));
